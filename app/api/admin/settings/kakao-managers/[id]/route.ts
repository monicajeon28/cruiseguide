...
export async function DELETE(
  ...
  try {
    ...
    const managers = (settings.kakaoApiManagers || []).filter((m: any) => m.id !== params.id);
    
    await writeSettingsFile({ ...settings, kakaoApiManagers: managers });

    return NextResponse.json({ ok: true });
  } catch (error) {
    console.error('[Kakao Managers DELETE] Error:', error);
    return NextResponse.json(
      { ok: false, error: error instanceof Error ? error.message : '담당자 삭제 중 오류가 발생했습니다.' },
      { status: 500 }
    );
  }
}
