...
import PartnerPassportRequestsClient from './PartnerPassportRequestsClient';

export default async function PartnerPassportRequestsPage({ params }: { params: { partnerId: string } }) {
  try {
    ...
    if (!isAdmin) {
      ...
      if (profile.user?.mallUserId !== partnerId) {
        redirect(`/partner/${profile.user?.mallUserId ?? ''}/passport-requests`);
      }
    ...
    return <PartnerPassportRequestsClient partnerId={partnerId} />;
  } catch (error) {
