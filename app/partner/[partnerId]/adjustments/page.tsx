...
import PartnerAdjustmentsClient from './PartnerAdjustmentsClient';

export default async function PartnerAdjustmentsPage({ params }: { params: { partnerId: string } }) {
  try {
    ...
    if (!isAdmin) {
      ...
      if (profile.user?.mallUserId !== partnerId) {
        redirect(`/partner/${profile.user?.mallUserId ?? ''}/adjustments`);
      }
    ...
    return <PartnerAdjustmentsClient currentUser={targetUser} profile={profile} />;
  } catch (error) {
