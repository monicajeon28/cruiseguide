...
export type Place = { code: string; name: string; q: string; emoji?: string; gmapsQuery?: string; country?: string; city?: string };
...
export const terminalsByRegion: Record<string, Place[]> = {
  ...
  TAIWAN: [
    { code:'KEELUNG', name:'기륭 크루즈 터미널', q:'Port of Keelung (Keelung Passenger Terminal)', emoji:'🛳️' },
...
// 모든 크루즈 터미널 리스트
const allCruiseTerminals = Object.values(terminalsByRegion).flat();

/** 토큰으로 공항 검색 */
export function findAirportsByToken(tokens: string[], countryHint?: string) {
  const results: Place[] = [];
  const targetAirports = countryHint ? airportsByCountry[countryHint] || [] : allAirports;

  for (const token of tokens) {
    const fuzzyMatch = targetAirports.filter(a => 
      a.name.toLowerCase().includes(token) ||
      a.q.toLowerCase().includes(token) ||
      (a.code && a.code.toLowerCase().includes(token))
    );
    results.push(...fuzzyMatch);
  }
  // 중복 제거 및 관련성 높은 순으로 정렬 (여기서는 간단히 유지)
  return Array.from(new Set(results));
}

/** 토큰으로 크루즈 터미널 검색 */
export function findCruiseTerminalsByToken(tokens: string[], countryHint?: string) {
  const results: Place[] = [];
  // countryHint를 사용하여 terminalsByRegion에서 필터링하거나, 전체 검색
  const targetTerminals = countryHint ? terminalsByRegion[countryHint.toUpperCase()] || [] : allCruiseTerminals;

  for (const token of tokens) {
    const fuzzyMatch = targetTerminals.filter(t => 
      t.name.toLowerCase().includes(token) ||
      t.q.toLowerCase().includes(token) ||
      (t.code && t.code.toLowerCase().includes(token))
    );
    results.push(...fuzzyMatch);
  }
...
/** 구글맵 쿼리용 장소명 정규화 */
export function normalizePlace(placeQuery: string) {
  // (예: "Taiwan Taoyuan International Airport (TPE)" → "Taiwan Taoyuan International Airport")
  return placeQuery.replace(/\s*\([^)]*\)/g, '').trim();
}
